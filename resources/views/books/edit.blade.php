<x-app-layout>
    ...
    <div class="max-w-2xl">
        <x-ui.card>
            ...
            <form action="{{ route('admin.books.update', $book->id) }}" method="POST" class="p-5 space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <x-ui.input name="judul" :value="old('judul', $book->judul)" required placeholder="Contoh: Laskar Pelangi" icon="ri-book-open-line" />
                            <x-input-error class="mt-1" :messages="$errors->get('judul')" />
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div>
                                <x-ui.label>Penulis</x-ui.label>
                                <x-ui.input name="penulis" :value="old('penulis', $book->penulis)" required placeholder="Andrea Hirata" icon="ri-user-line" />
                                <x-input-error class="mt-1" :messages="$errors->get('penulis')" />
                            </div>
                            <div>
                                <x-ui.label>Penerbit</x-ui.label>
                                <x-ui.input name="penerbit" :value="old('penerbit', $book->penerbit)" required placeholder="Bentang Pustaka" icon="ri-building-2-line" />
                ...
                <div class="border-t border-amber-100 pt-5">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <x-ui.input type="number" name="tahun_terbit" :value="old('tahun_terbit', $book->tahun_terbit)" required min="1900" max="{{ date('Y') + 1 }}" placeholder="2024" icon="ri-calendar-line" />
                            <x-input-error class="mt-1" :messages="$errors->get('tahun_terbit')" />
                        </div>
                        <div>
                            <x-ui.label>Jumlah Stok</x-ui.label>
                            <x-ui.input type="number" name="stok" :value="old('stok', $book->stok)" required min="0" max="10000" placeholder="10" icon="ri-stack-line" />
