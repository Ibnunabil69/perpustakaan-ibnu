        ...
        <x-ui.card>
            ...
            <div class="bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 rounded-t-2xl px-5 py-4 flex items-center gap-3">
                <div class="w-9 h-9 bg-white/20 rounded-lg flex items-center justify-center backdrop-blur-sm">
                    <i class="ri-edit-2-line text-white text-lg"></i>
                </div>
                <div>
                    <h3 class="text-sm font-semibold text-white">Edit Buku</h3>
                    <p class="text-xs text-white/70">{{ $book->judul }}</p>
                </div>
            </div>

            <form action="{{ route('admin.books.update', $book->id) }}" method="POST" enctype="multipart/form-data" 
                class="p-6 space-y-6" x-data="{ imageUrl: '{{ $book->cover ? asset('storage/' . $book->cover) : '' }}' }">
                @csrf
                @method('PUT')

                <div class="grid grid-cols-1 md:grid-cols-12 gap-8">
                    <!-- Left: Cover Preview (Same as create) -->
                    <!-- Left: Cover Preview (Same as create) -->
                    <div class="md:col-span-3 flex flex-col items-center gap-3">
                        ...
                        <!-- Preview Box -->
                        <div class="relative group aspect-[3/4] w-full max-w-[160px] rounded-2xl bg-gray-50 border-2 border-dashed border-gray-200 flex flex-col items-center justify-center overflow-hidden transition-all hover:border-amber-300 shadow-sm bg-white">
                        ...
                        <div class="text-center space-y-1">
                            ...
                            <p class="text-[10px] text-amber-600 font-medium">Biarkan kosong jika tidak ingin mengubah sampul.</p>
                        </div>
                    ...
                    <div class="md:col-span-9 flex flex-col gap-5">
                        <div class="grid grid-cols-1 gap-4">
                            ...
                            <div>
                                ...
                                <x-ui.input id="judul" name="judul" :value="old('judul', $book->judul)" required placeholder="Isi Judul Buku..." icon="ri-book-open-line" />
                            ...
                            <div>
                                ...
                                <x-ui.select id="category_id" name="category_id" :value="old('category_id', $book->category_id)" placeholder="Pilih Kategori">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <x-ui.input id="penulis" name="penulis" :value="old('penulis', $book->penulis)" required placeholder="Tulis Nama Penulis..." icon="ri-user-line" />
                                    <x-input-error class="mt-1" :messages="$errors->get('penulis')" />
                                </div>
                                <div>
                                    <x-ui.label for="penerbit">Penerbit</x-ui.label>
                                    <x-ui.input id="penerbit" name="penerbit" :value="old('penerbit', $book->penerbit)" required placeholder="Tulis Nama Penerbit..." icon="ri-building-line" />
                ...
                <div class="space-y-6 pt-6 border-t border-gray-100">
                    <div>
                        ...
                        <textarea id="deskripsi" name="deskripsi" rows="3" 
                            class="w-full rounded-2xl border-gray-200 focus:border-amber-300 focus:ring-amber-300 text-sm p-4 transition-all" 
                            placeholder="Tuliskan ringkasan singkat buku ini...">{{ old('deskripsi', $book->deskripsi) }}</textarea>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-ui.input type="number" id="tahun_terbit" name="tahun_terbit" :value="old('tahun_terbit', $book->tahun_terbit)" required placeholder="Tahun Terbit Buku..." icon="ri-calendar-line" />
                            <x-input-error class="mt-1" :messages="$errors->get('tahun_terbit')" />
                        </div>
                        <div>
                            <x-ui.label for="stok">Stok Buku</x-ui.label>
                            <x-ui.input type="number" id="stok" name="stok" :value="old('stok', $book->stok)" required placeholder="Jumlah Stok..." icon="ri-stack-line" />
                ...
                <div class="flex items-center justify-end gap-3 pt-6 border-t border-gray-100">
                    ...
                    <x-ui.button type="submit">
                        <i class="ri-save-line mr-1"></i> Perbarui Buku
                    </x-ui.button>
                </div>
            </form>
        </x-ui.card>
